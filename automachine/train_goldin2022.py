...
def evaluate(model, path, batch_size, device, max_batches, mean_response):
    model.eval()
    model_loss = baseline_loss = 0.0
    batches_seen = 0
    baseline = torch.log(mean_response.clamp_min(1e-6)).view(1, 1, -1)
    for video, target in batches(path, batch_size, False):
        video, target = video.to(device), target.to(device)
        prediction = model(video).clamp(max=10.0)
        model_loss += poisson_loss(prediction, target).item()
    ...
    model.train()
    if not batches_seen:
        raise RuntimeError(f"No evaluation batches produced from {path}")
    return model_loss / batches_seen, baseline_loss / batches_seen
...
def train(args: argparse.Namespace) -> None:
    ...
    if args.resume:
        ...
        if saved_config is not None and saved_config != config:
            raise ValueError(f"Checkpoint config differs from requested config: {saved_config} != {config}")
            ...
            for video, target in batches(args.data / "train", args.batch_size, True):
                ...
                if step % args.eval_every == 0:
                    val, baseline = evaluate(
                        model, args.data / "validation", args.batch_size, device,
                        args.validation_batches, mean_response,
                    )
                    print(
                        f"step={step:05d} val_poisson={val:.6f} "
                        f"mean_baseline={baseline:.6f} better={val < baseline}"
                    )
    ...
    test, baseline = evaluate(
        model, args.data / "test", args.batch_size, device, args.validation_batches, mean_response
    )
    print(f"test_poisson={test:.6f} mean_baseline={baseline:.6f} better={test < baseline}")
    save_checkpoint(args.checkpoint, model, optimizer, config, step, args, manifest)
